...
1. Full sweep: go through every STEP3_PRODUCTION_SPEC.md section once more hunting for reduction candidates. If any are found, present them — the step is not ready to close; go back to /simplify-spec.
2. Preservation check against ../step_02_spec_extraction/STEP2_VIBE_SPEC.md: every behavior of the raw spec must be either present in the simplified spec or accounted for by an entry in STEP3_SPEC_OPTIMISATION.md (including PRODUCT entries for user-decided changes). List anything unaccounted for and resolve it with the user, one by one.
