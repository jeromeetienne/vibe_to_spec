...
Goal: recover the architecture that was actually built, by turning the validated prototype of step 1 into SPEC.md — the raw specification.

The deliverable of this step is SPEC.md, and nothing else. No product code is written in this step.
...
## SPEC.md — required structure
    ...
    ## Known gaps         — the gaps the user explicitly accepted in step 1, carried over from DECISIONS.md
...
Never classify silently. A wrong guess here corrupts the source of truth for every later step.
...
Step 2 ends ONLY when the user explicitly agrees that SPEC.md fully accounts for the prototype. Use /close-step to run the closing walkthrough. The agreement is recorded as the Status line of SPEC.md:
