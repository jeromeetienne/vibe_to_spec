...
- **Inputs, read-only**:
  ...
  - the implementation — the subject under verification, at the external location recorded by the `Implementation:` line of `../step_04_implementation/SPECGAPS.md`
...
2. **Check each item against the implementation** and record a verdict in `VERIFICATION.md`: conforms / deviates (and how) / missing.
...
- **Gaps found** → the gap list goes back to step 4; after the fixes, verification runs again. The loop repeats until `VERIFICATION.md` shows full conformance.
