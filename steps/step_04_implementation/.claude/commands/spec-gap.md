...
1. Create SPECGAPS.md if it does not exist yet: the `# Specification gaps` heading plus the `Implementation: <link or absolute path>` line (ask the user for the location if it is not known yet).
...
4. WAIT for the user's decision. Then:
   ...
   - if the resolution changes the specification: apply the agreed fix to ../step_03_spec_simplification/SPEC.md and add `Amended on <date>: <one-line summary>` under its Status line;
   - continue implementing from the amended specification.

Do not write to any file outside this folder except ../step_03_spec_simplification/SPEC.md, and only for the agreed fix.
