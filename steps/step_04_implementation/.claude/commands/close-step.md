---
description: Run the phase-closing ritual — resolve any still-open spec gaps, walk the implementation against every specification section, run the full test suite for real, then ask the final question "Does this implementation cover the specification, with passing tests?". Close only on the user's explicit yes, recorded as the CLOSED entry in SPECGAPS.md.
