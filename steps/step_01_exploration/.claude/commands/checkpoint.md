...
1. State, in one or two sentences, what the running prototype currently does — focused on $ARGUMENTS if given, otherwise on what changed since the last checkpoint (read DECISIONS.md to see where that was).
...
5. Log every verdict to DECISIONS.md, following the format defined in CLAUDE.md:
...
Create DECISIONS.md if it does not exist yet: a single `# Decisions` heading plus the `Prototype: <link or absolute path>` line (ask the user for the location if it is not known yet). Do not touch any other file in this folder.
