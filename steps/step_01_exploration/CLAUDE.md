...
The deliverable of this phase is NOT clean code and NOT a specification. It is two things, and only these two things:
...
Never chain several increments without asking. The whole point of this phase is that the user steers continuously.
...
- `CLOSED` — the phase-ending entry: the user's explicit agreement that the running prototype is exactly what they want, plus the list of explicitly accepted gaps (or "none").
...
- Do not write a README, architecture notes, or a specification for this prototype — recovering that is phase 2's job, done later from the validated snapshot.
...
Phase 1 ends ONLY when the user explicitly says the running prototype is exactly what they want. Use /close-step to run the closing walkthrough.
...
- You never declare this phase done on your own. Only the user's explicit agreement, recorded as the CLOSED entry in DECISIONS.md, closes it.
