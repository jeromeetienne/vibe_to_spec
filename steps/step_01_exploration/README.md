...
- **The step closes ONLY on the user's explicit yes**, recorded as the `CLOSED` entry in `DECISIONS.md`, with the accepted gaps listed (or "none"). The agent never closes the step on its own.
- **Hand-off**: the validated prototype stays in its external repository or folder; `DECISIONS.md` — which records where that is — stays in this folder. Step 2 (`../step_02_spec_extraction`) reads both, read-only.
